...
namespace jopp2
{
	...
	template<class T>
	struct wrap_in_variant
	{
		using type = std::variant<T>;
	};

	template<class ... Args>
	struct wrap_in_variant<std::variant<Args...>>
	{
		using type = std::variant<Args...>;
	};

	template<class T>
	using wrap_in_variant_t = wrap_in_variant<T>::type;

	template<class A, class B>
	using build_variant_t = concatenate_variants_t<wrap_in_variant_t<A>, wrap_in_variant_t<B>>;
